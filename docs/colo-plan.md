...
| v4.15 | 20 Sep 2026 | **Review of the whole codebase, a pass over every feature, and the fixes it found.** Six real faults, each now covered by a test that fails without its fix. **A title the list could not take stranded its document:** `updateMeta` is the machine path behind a save, and it *refused* a title over 200 characters instead of cutting it — an imported Word title has no length limit — so the rejected RPC left the object re-arming its alarm every minute and the document list never saw that document's title or edit time again (`clampTitle`). **A saved state that would not load bricked its document for good:** partyserver starts an object from `fetch`, so a throw in `onLoad` failed *every* request to it, including the restore routes that would have repaired it. Such a document now opens empty and read-only, says so (`DOCUMENT_UNREADABLE`), is never saved over, and comes back with a restore point or a backup. `isEmptyState` stopped throwing on the save path for the same reason. **A backup of a document over 1.9 MB could tear:** each `yield` suspends until the reader pulls, so a debounced save could land between two chunk reads and put halves of two different states in one file that still looked complete; a save during the export now fails it, which the missing `end` record already makes detectable. Also: a restore keeps the whole-document image cap, a damaged backup names the line of the *file* rather than the nth non-blank one, `--url http://localhost.example.com` no longer passes the script's localhost gate, and a cancelled download no longer leaves a document's stream open. **Accessibility:** the account button labelled itself "Account" over its visible name, which left voice control unable to say it (WCAG 2.5.3). **The table picker never worked with a dragged finger** — the browser read the drag as a pan and sent `pointercancel`, and the implicit capture was released on the wrong element; `touch-action: none` and releasing it on the cell fix it, and `e2e:mobile` now drags rather than only tapping. **`npm run e2e:auth` had been broken since `ea0195a`** (M8), which moved Sign out into the account menu without updating the M1 test — it was listed as passing in §11. Every suite passes: 139 unit tests and all ten browser suites |
| v4.16 | 20 Sep 2026 | **A footer on the document list, with the mark in liquid metal.** The page became a flex column so the footer is pushed to the bottom of the window on a short list instead of crowding under it, and sits at the end of the page on a long one. The mark is `logo.svg` under the Liquid Metal shader from `@paper-design/shaders-react` (Apache-2.0, bundled like every other dependency — no CDN, no service): the shader takes the image's alpha as its shape mask, so the vector outline stays the single source of truth for the logo. Decoration is not allowed to cost the screen anything: the shader is a lazy `import()` (a 12 KB gzipped chunk once tree-shaken, not the whole library) that is fetched only when the footer comes near the window, the animation stops while it is off screen, and the plain SVG renders first and simply stays put under reduced motion, without WebGL, or if the chunk fails to load. The shader needs no `worker-src` in the CSP — it uses neither workers nor blob URLs. |
| v5 | 23 Sep 2026 | **Workspaces, password sign-in and an owner's dashboard (M9, [ADR 0006](decisions/0006-workspaces-and-password-sign-in.md)).** Invite links are gone. One Workspace object per workspace; a new `Admin` object is the registry (slug → object, member cap, disabled flag) and holds the owner's passkey. The session cookie names its workspace object, so only sign-in asks `Admin` anything. Members sign in with workspace ID, email and password: PBKDF2-SHA256 at the runtime's 100,000-iteration maximum, run in the Workspace object, with a 15-minute lockout after five failures and one `LOGIN_FAILED` for every failure. The owner sets or generates passwords at `/admin`, sees each once, and can reset or disable; members can change their own. Isolation is the existing index check — a document another workspace made is a 404 — plus `doc_workspace` in each Document object so metadata and restores go to the workspace that owns it; a restore may not claim a document another workspace holds. The pre-M9 workspace is `main` on its old object, its members kept without passwords until the owner sets them |
...
| Custom strings (`__YPS:` prefix) | Server → client | JSON control events: `session-expired`, `document-deleted`, `restored` (by whom), `limit` (`MESSAGE_TOO_LARGE`, `RATE_LIMITED`, `DOCUMENT_TOO_LARGE`, `DOCUMENT_UNREADABLE`) |
...
- **Caps (per connection, enforced in the Document object before Yjs processing).**
  ...
  - A saved state that will not apply makes the document read-only with a `DOCUMENT_UNREADABLE` notice, and the object refuses to save over it. An object is started by `fetch`, so throwing while loading would fail every request to it — including the restore routes that repair it (v4.15).
...
**Since v5 (M9, [ADR 0006](decisions/0006-workspaces-and-password-sign-in.md)):** members sign in to one workspace with its ID, their email and a password the owner set (`POST /api/auth/login`); the Worker asks the `Admin` object which workspace object the ID names and forwards the request there. Passwords are PBKDF2-SHA256, 100,000 iterations, 16-byte salt, verified in the Workspace object; five failures lock an account for 15 minutes; every failure is `LOGIN_FAILED`. `POST /api/auth/password` changes one's own password and ends one's other sessions. The session cookie is `__Host-colo_session=<workspace object>.<token>`, so the Worker routes by it and a token is only ever looked up in the workspace it names. The owner's dashboard (`/admin`, `/api/admin/*`) signs in with the owner's passkey — enrolled from a one-time link made by `npm run admin:enroll` with `ADMIN_TOKEN` — into a 12-hour `__Host-colo_admin` session that never slides.

Until v5, and still for the owner's passkey ([ADR 0002](decisions/0002-passkey-auth-on-workers-dev.md)): invite-only passkeys with `@simplewebauthn/server` (M1 spike confirms it runs in workerd; WebCrypto fallback), discoverable credentials, user verification required, `__Host-colo_session` cookie with a 30-day sliding expiry stored as a SHA-256 hash, `Origin` checks, and an `ADMIN_TOKEN` secret deleted after both users enrol. Relying party ID `colo.manan-vala.workers.dev`; passkeys are bound to that hostname.
...
| Footer logo | `@paper-design/shaders-react` (Apache-2.0), loaded on first sight of the footer | The Liquid Metal shader over `logo.svg`; masked by the image's alpha, so the logo stays one vector. Reduced motion, no WebGL or a failed chunk all fall back to the plain SVG |
  ...
  src/
    client/
      ...
      convert/              # import and export (loaded on first use): docx/ reader and writer,
                            ...
                            # formats.ts (the format lists, loaded with the menus), pending.ts
      lib/utils.ts
    worker/
...
# Admin secret: keep a copy outside the repo (the enrollment script reads ~/.colo/admin-token)
mkdir -p ~/.colo && openssl rand -base64 32 | tr -d '
' > ~/.colo/admin-token
npx wrangler secret put ADMIN_TOKEN < ~/.colo/admin-token

npm run admin:enroll   # one-time link for the owner's dashboard passkey, valid for 24 h
# Then, at /admin: set passwords for main's members, create workspaces, add members.

# After the owner's passkey is enrolled
...
**A document over 1.9 MB is not exported while it is being saved.** Its state is several rows, and each record yielded suspends the export until the browser pulls the next one, so a save landing between two chunk reads would put half of one state and half of another in a file that still ended with its `end` record. A save during such a document's export therefore fails the export instead: the file stops without its `end` record, which is exactly what marks a backup truncated. One chunk is one statement, so an ordinary document cannot tear and is never refused (v4.15).
...
# 2. Apply the backup into a workspace (default: main). --overwrite is needed if it already has documents.
npm run restore -- --file colo-backup-2026-09-20.ndjson --workspace main --overwrite

# 3. Passwords are not in a backup: enrol the owner's passkey, then set each member a password at /admin.
npm run admin:enroll -- --local
```

Members keep their ids — and therefore their documents' `created_by` and their cursor colours. A restore refuses (`DOCUMENT_IN_OTHER_WORKSPACE`) a document that already holds content in a different workspace, before writing its index row, so one backup cannot make two workspaces share a document.

A restore that stops part-way leaves a document holding chunks from two different states. That document opens empty and read-only with a `DOCUMENT_UNREADABLE` notice rather than failing every request to its object, and nothing is saved over it, so repeating the restore — or restoring a point — brings it back (§4.3, v4.15).

Two properties do the safety work:

- **`POST /api/admin/restore` does not exist in production.** It sits behind `requireAdmin`, which answers 404 while `ADMIN_TOKEN` is unset, and production deletes that secret once both passkeys are enrolled (§8.2). The script also refuses a non-localhost target without `--force`, comparing the URL's hostname rather than its opening characters — `http://localhost.example.com` belongs to someone else (v4.15).
- **Ids are preserved, never reminted.** An image's `src` embeds its document id (`imagePath`), so a restore that minted new ids would break every image in every document. Every write is an upsert keyed on the id from the backup, so running the same file twice changes nothing and a run that stopped part-way can be repeated — with `--overwrite`, since by then the instance holds what the first attempt wrote. Nothing is ever deleted: rows in the backup win, rows not in it are left alone.
...
- **Server cannot validate document structure** cheaply; a corrupt update affects everyone. Mitigated by restore points and the `pre-restore` safety copy. A stored state that will not load at all no longer takes its document with it: it opens read-only and is never written over, so a restore point or a backup brings it back (§4.3).
...
| **M8 — Hardening** ✅ built | `/api/export`; PITR check; security review; Workers Builds CI; metrics review; mobile pass | **Backups:** `npm test` covers the format, the SQL and the refusals, including an allow-list that stops a passkey or session ever being written; `npm run e2e:backup` is the acceptance check — a document with text and an image is exported through the account menu (90 MB reached disk), wrecked, restored with `npm run restore`, and comes back with its image still serving from the same URL, which proves the id survived, and without being counted as an edit. **Mobile:** `npm run e2e:mobile` walks the app at 390×844 with touch; fixed the save/offline status being hidden on phones, the table picker giving no touch feedback, 12 px resize handles and the safe-area inset. **Security review:** no exploitable finding; two robustness fixes; LICENSE and SECURITY.md added for the public repo. **CI:** `npm run ci` verified from a clean clone — the dashboard side is still to connect. **PITR:** answered as far as it can be without deploying a probe (§8.5), and no longer depended on. **Review (v4.15):** the whole codebase re-read and every feature exercised — six faults fixed, each pinned by a test that fails without its fix, and `e2e:auth`, broken since M8 moved Sign out into the account menu, repaired. **Still pending:** connect Workers Builds, deploy, the first metrics reading, and a real-phone check (passkey sign-in especially) | 2 days |
| **M9 — Workspaces** ✅ built | `Admin` object and workspace registry; password sign-in replacing invites; routing by the workspace in the session cookie; owner's dashboard at `/admin`; change password; workspace-scoped restore ([ADR 0006](decisions/0006-workspaces-and-password-sign-in.md)) | `npm test` (147): password sign-in, lockout, identical failures, cookies that cannot move between workspaces, member caps, reset and disable ending sessions, slug renames keeping sessions, a disabled workspace signing everyone out, a member of one workspace getting 404 on another's document, socket and restore points, metadata reaching a new workspace, a restore refused across workspaces with no index row left behind, no password material in a backup. `e2e:auth` (owner adds a member in the dashboard → sign in → wrong password → change password → old one refused), `e2e:collab` and `e2e:backup` ✅ on the production build. **Still pending:** deploy, time a sign-in's CPU on the deployed Worker, enrol the owner's passkey and set main's passwords | 1 day |
...
| D7 | Per-document sharing or roles | Deferred; all members of a workspace edit all of its documents. Workspaces (v5) are the only boundary |
